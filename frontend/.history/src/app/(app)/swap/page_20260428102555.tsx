...
export default function SwapPage() {
  const { wallets } = useWallet() as any;
  ...
  const canSwap = fromCurrency && toCurrency && fromSym !== toSym && parsedFromAmount > 0 && !exceedsBalance && rateData && !rateLoading;

  const handleSwapClick = async () => {
    setError("");
    if (!canSwap) return;
    const minSwap = MIN_SWAP[fromSym?.toUpperCase()];
    if (minSwap && parsedFromAmount < minSwap) {
      setError(`Minimum swap amount for ${fromSym} is ${minSwap} ${fromSym}`);
      return;
    }
