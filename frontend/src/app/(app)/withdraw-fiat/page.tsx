...
let _banksCache: any[] | null = null;

const FALLBACK_TIERS = [
  { min_amount: 0, max_amount: 9999.99, fee: 20, stamp_duty: 0, total_fee: 20 },
  { min_amount: 10000, max_amount: null, fee: 20, stamp_duty: 50, total_fee: 70 },
];
...
export default function WithdrawFiatPage() {
  ...
  const senderName = user?.full_name || user?.name || [user?.first_name, user?.middle_name, user?.last_name].filter(Boolean).join(" ") || user?.email || "";
  ...
  useEffect(() => {
    loadBanks();
    walletService.getWithdrawalFees().then((data: any) => {
      const tiers = data?.fiat?.tiers;
      if (Array.isArray(tiers) && tiers.length > 0) {
        setFiatFeeTiers(tiers.map((t: any) => ({
          min_amount: parseFloat(t.min_amount || "0"),
          max_amount: t.max_amount != null ? parseFloat(t.max_amount) : null,
          fee: parseFloat(t.fee || "0"),
          stamp_duty: parseFloat(t.stamp_duty || "0"),
          total_fee: parseFloat(t.total_fee || "0"),
        })));
      }
  ...
  const loadBanks = async () => {
    if (_banksCache) { setBanks(_banksCache); return; }
        ...
        {feeInfo && parsedAmount > 0 && (
          <div className="bg-[#F0F7FF] border border-[#D6E4F0] rounded-xl p-4 space-y-2">
            ...
            {feeInfo.stamp_duty > 0 && (
              <div className="flex justify-between">
                <span className="text-xs text-[#555]">Transfer Levy</span>
