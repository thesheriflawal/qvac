...
import { ArrowLeft, ShieldCheck, AlertCircle, PlusCircle, X } from "lucide-react";
...
function BuyCryptoContent() {
  ...
  const MIN_LIMIT = ad?.min_amount ? parseFloat(ad.min_amount) : 5000;
  // For non-rollover: remaining_quantity = remaining ad capacity. For rollover: remaining_quantity reflects creator's live balance.
  const MAX_LIMIT = parseFloat(ad?.remaining_quantity || "0") || parseFloat(ad?.total_quantity || "0");
  // EFFECTIVE_MAX accounts for taker fee — what the taker can actually input. null means no frontend cap (let backend validate).
  ...
  const ngnWallet = getWalletByCurrency("NGN");
  const AVAILABLE_NGN = Math.max(0, parseFloat(String(ngnWallet?.balance || 0)) - parseFloat(String(ngnWallet?.locked_balance || 0)));
  ...
  const platformFeeCents = Math.round(Math.round(fiatTotal * 100) * takerFeeRate);
  const platformFee = platformFeeCents / 100;
  const totalToPay = (Math.round(fiatTotal * 100) + platformFeeCents) / 100;

  const handleMax = () => {
    if (isFiat) {
      // Input in NGN. Total paid = amount * (1 + feeRate) must not exceed balance, so max = balance / (1 + feeRate).
      // Also cap at ad's NGN limit. Floor to 2dp so rounding never pushes total over balance.
      const maxAfterFee = AVAILABLE_NGN / (1 + takerFeeRate);
      const adMaxNGN = EFFECTIVE_MAX != null ? EFFECTIVE_MAX * PRICE : Infinity;
      const max = Math.min(maxAfterFee, adMaxNGN);
      setAmount((Math.floor(max * 100) / 100).toFixed(2));
    } else {
      // Input in crypto. Total NGN = amount * PRICE * (1 + feeRate) must not exceed balance.
      // Also cap at ad's crypto limit. Floor so rounding never causes balance overshoot.
      const balCrypto = AVAILABLE_NGN / (PRICE * (1 + takerFeeRate));
      const max = EFFECTIVE_MAX != null ? Math.min(balCrypto, EFFECTIVE_MAX) : balCrypto;
  ...
  const handleBuy = () => {
    ...
    const cryptoVal = isFiat ? parsedAmount / PRICE : parsedAmount;
    if (cryptoVal < MIN_LIMIT || (EFFECTIVE_MAX != null && cryptoVal > EFFECTIVE_MAX)) { setErrorType("LIMIT"); return; }
    const ngnVal = isFiat ? parsedAmount : parsedAmount * PRICE;
    if (ngnVal * (1 + takerFeeRate) > AVAILABLE_NGN) { setErrorType("BALANCE"); return; }
    setDisclaimer(true);
  };
  ...
  const onPin = async (pin: string) => {
    ...
    try {
      ...
      setOrderResult({
        type: "Buy", amount: `${amount} ${isFiat ? "NGN" : adCurrency}`,
  ...
  return (
    <div className="w-full pb-32">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-sm text-[#8E8E93]">P2P / <span className="text-[#1D3B53]">buy {adCurrency}</span></span>
      </div>
      ...
      <div className="flex items-center justify-between mb-3">
        <h1 className="font-bold text-xl text-[#1D3B53]">Buy {adCurrency}</h1>
      ...
      {/* Price */}
      <p className="text-sm font-semibold text-[#1D3B53] mb-2">
        Price: <span className={errorType === "LIMIT" ? "text-[#D92D20] line-through" : "text-[#5CB85C]"}>{PRICE.toFixed(2)} NGN</span>
      </p>

      {/* Withdrawal Security Protection Banner */}
      {showSecurityBanner && (
        <div className="flex items-start gap-3 bg-[#EBF4FF] border border-primary/20 rounded-xl p-4 mb-4">
          <ShieldCheck size={18} className="text-[#5CB85C] shrink-0 mt-0.5" />
          <div className="flex-1">
            <p className="text-sm font-semibold text-[#1D3B53]">Withdrawal Security Protection</p>
            <p className="text-xs text-[#8E8E93]">Fiat withdrawal is only after KYC is verified.</p>
          </div>
          <button onClick={() => setShowSecurityBanner(false)} className="text-gray-400 hover:text-gray-600 cursor-pointer">
            <X size={16} />
          </button>
        </div>
      )}
      ...
      <div className="flex bg-[#F7F9FC] rounded-xl p-1 w-fit mb-5">
        {(["Fiat", "Crypto"] as const).map(t => (
          <button key={t} onClick={() => { setPaymentTab(t); setAmount(""); }}
            className={`px-5 py-2 rounded-lg text-sm font-semibold cursor-pointer transition-all ${paymentTab === t ? "bg-white shadow text-primary" : "text-[#8E8E93]"}`}
          >
            With {t}
          </button>
        ))}
      ...
      {/* Input Card */}
      <div className="bg-[#EBF4FF] rounded-2xl p-5 border border-primary mb-4">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button onClick={handleMax} className="text-sm text-primary font-semibold border-l border-gray-300 pl-2 cursor-pointer">Max</button>
          </div>
        </div>
        <div className="h-px bg-gray-200 mb-3" />
        <div className="flex justify-between text-xs mb-1">
          <span className="text-[#8E8E93]">
            Limits: <span className="text-[#1D3B53] font-medium">{MIN_LIMIT.toLocaleString("en-US", { minimumFractionDigits: 2 })} - {EFFECTIVE_MAX != null ? EFFECTIVE_MAX.toLocaleString("en-US", { minimumFractionDigits: 2 }) : "—"} {adCurrency}</span>
          </span>
          <span className="text-primary">Fee: {takerFeeDisplay}</span>
        </div>


        <p className="text-sm font-semibold text-[#1D3B53]">
          {isFiat ? `I will Receive: ${calculatedValue} ${adCurrency}` : `I will Pay: ${calculatedValue} NGN`}
        </p>
      </div>

      {/* Balance Card */}
      <div className={`bg-[#EBF4FF] rounded-2xl p-4 mb-5 flex justify-between items-center border ${errorType === "BALANCE" ? "border-[#D92D20]" : "border-gray-200"}`}>
        <div>
          <p className="text-xs text-[#8E8E93]">
            Available Balance:{" "}
            <span className="text-primary font-semibold text-sm">{AVAILABLE_NGN.toLocaleString("en-US", { minimumFractionDigits: 2 })}</span>{" "}NGN
          </p>
        ...
        <div className="max-w-lg mx-auto flex items-center gap-4">
          <div className="shrink-0">
            <div className="flex items-end gap-1">
              <span className="text-2xl font-bold text-black">{totalToPay > 0 ? totalToPay.toLocaleString("en-US") : amount || "0"}</span>
              <span className="text-sm text-[#8E8E93] mb-0.5">{isFiat ? "NGN" : adCurrency}</span>
            </div>
            <p className="text-xs text-[#8E8E93]">{isFiat ? "Total Payable" : "Total Quantity"}</p>
          </div>
          <button
            onClick={handleBuy}
            disabled={loading || !amount}
            className="flex-1 bg-primary text-white font-bold py-3.5 rounded-2xl cursor-pointer disabled:opacity-50 text-center"
          >
            {loading ? "..." : "Buy"}
          </button>
        </div>
...
export default function BuyCryptoPage() {
  return <Suspense><BuyCryptoContent /></Suspense>;
}
