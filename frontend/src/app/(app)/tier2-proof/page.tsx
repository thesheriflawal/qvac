...
export default function KYCPage() {
  ...
  const title = TITLES["tier2-proof"] || "Verification";
  const [value, setValue] = useState(""); const [loading, setLoading] = useState(false); const [error, setError] = useState("");
  const handle = async () => { setLoading(true); setError(""); try { await kycService.submitTier2({ nin: value }); router.push("/verification-dashboard"); } catch(e) { setError(getErrorMessage(e)); } finally { setLoading(false); } };
  return (
    <div>
      <button onClick={() => router.push("/tier2-address")} className="flex items-center gap-2 text-gray-500 mb-4 cursor-pointer"><ArrowLeft size={18}/>Back</button>
