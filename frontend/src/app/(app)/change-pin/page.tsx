  ...
  return (
    <div className="grid grid-cols-3 bg-gray-200">
      {keys.map((k, i) => {
        ...
        return (
          <button key={i} onClick={() => onKey(k)} className="py-5 text-xl font-semibold text-[#1D3B53] cursor-pointer active:bg-gray-300 border-r border-t border-gray-300 last:border-r-0">
            {k}
          </button>
        );
      })}
...
export default function ChangePinPage() {
  ...
  const [active, setActive] = useState<Field>("old");
  ...
  const handleKey = (val: string) => {
    if (active === "old") {
      if (oldPin.length < 6) {
        const updated = oldPin + val;
        setOldPin(updated);
        if (updated.length === 6) setActive("new");
      }
    } else if (active === "new") {
      if (newPin.length < 6) {
  ...
  const handleDelete = () => {
    if (active === "old") setOldPin(p => p.slice(0, -1));
    else if (active === "new") {
      if (newPin.length === 0) setActive("old");
      else setNewPin(p => p.slice(0, -1));
    } else {
      if (confirmPin.length === 0) setActive("new");
  ...
  const handleContinue = async () => {
    if (oldPin.length < 6) {
      setStatus({ visible: true, type: "error", title: "Error", message: "Please enter your current PIN" }); return;
    }
    if (newPin.length < 6 || confirmPin.length < 6) {
      setStatus({ visible: true, type: "error", title: "Error", message: "Please complete all PIN fields" }); return;
    }
    if (newPin !== confirmPin) {
      setStatus({ visible: true, type: "error", title: "Error", message: "New PINs do not match" });
      setConfirmPin(""); setActive("confirm"); return;
    }
    setLoading(true);
    try {
      await securityService.changePin(oldPin, newPin, newPin);
      router.push("/change-pin-success");
    } catch (e) {
      const msg = getErrorMessage(e) || "Failed to change PIN. Please check your current PIN.";
      setStatus({ visible: true, type: "error", title: "Error", message: msg });
      setOldPin(""); setNewPin(""); setConfirmPin(""); setActive("old");
    } finally { setLoading(false); }
  };

  const sections: { field: Field; label: string; subtitle: string; value: string }[] = [
    { field: "old", label: "Current Pin", subtitle: "Enter your current PIN", value: oldPin },
    { field: "new", label: "Enter New Pin", subtitle: "Enter your new Pin", value: newPin },
    { field: "confirm", label: "Confirm New Pin", subtitle: "Enter your new pin again", value: confirmPin },
  ];
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        ...
        <span className="font-bold text-lg text-[#1D3B53]">Change Pin</span>
        <div className="w-8" />
      </div>

      {/* PIN Sections */}
      <div className="flex-1 px-5 pt-6 pb-4 space-y-6">
        {sections.map(s => (
